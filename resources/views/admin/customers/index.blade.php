...
<div class="space-y-6">
    ...
    <div class="exec-card bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden">
        ...
        <div class="px-6 py-5 bg-white border-b border-slate-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <a href="{{ route('admin.customers.create') }}" class="btn-exec-primary rounded-xl px-5 py-2.5 text-xs transition-all shadow-md flex items-center gap-2 cursor-pointer" style="background-color: #064e3b !important; color: #ffffff !important; font-weight: 700 !important;">
        ...
        <div class="p-5 sm:p-6 bg-slate-50 border-b border-slate-200 space-y-4">
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4 text-xs font-semibold text-slate-700">
                ...
                <!-- Left: Tier Filter & Full Width Search Form -->
                <form action="{{ route('admin.customers') }}" method="GET" class="flex flex-wrap items-center gap-2.5 w-full sm:flex-1 sm:max-w-xl">
                    <select name="vip_tier" onchange="this.form.submit()" class="h-10 px-3.5 rounded-xl border border-slate-300 bg-white text-xs font-semibold text-slate-800 outline-none focus:border-emerald-600 focus:ring-2 focus:ring-emerald-600/20 shadow-2xs" style="background-color: #ffffff !important; color: #0f172a !important; border: 1px solid #cbd5e1 !important;">
                    ...
                    <div class="relative flex-1 min-w-[200px]">
                        <input
                            ...
                            placeholder="Kunde nach Name, E-Mail oder Telefon suchen..."
                            data-i18n-placeholder-de="Kunde nach Name, E-Mail oder Telefon suchen..."
                            data-i18n-placeholder-en="Search customer by name, email or phone..."
                    ...
                    @if(request('search') || request('vip_tier'))
                        <a href="{{ route('admin.customers') }}" class="btn-exec-secondary px-3.5 py-2 rounded-xl text-xs transition font-bold" data-i18n-de="Zurücksetzen" data-i18n-en="Clear">Clear</a>
                    @endif
                </form>
            ...
            <table class="w-full text-left text-xs">
                <thead>
                    <tr class="exec-table-head bg-slate-100 text-slate-800 font-extrabold uppercase tracking-wider text-[0.7rem] border-y border-slate-300" style="background-color: #f1f5f9 !important; color: #1e293b !important; border-bottom: 2px solid #cbd5e1 !important;">
                        <th class="py-4 px-6 font-black" data-i18n-de="Kunden Name & E-Mail" data-i18n-en="Customer Name & Email">Kunden Name & E-Mail</th>
                        <th class="py-4 px-6 font-black" data-i18n-de="Telefon & Stadt" data-i18n-en="Phone & City">Telefon & Stadt</th>
                        <th class="py-4 px-6 font-black" data-i18n-de="VIP Tier Status" data-i18n-en="VIP Tier Status">VIP Tier Status</th>
                        <th class="py-4 px-6 font-black" data-i18n-de="Gesamtumsatz (€)" data-i18n-en="Total Spent (€)">Gesamtumsatz (€)</th>
                        <th class="py-4 px-6 font-black" data-i18n-de="Bestellungen" data-i18n-en="Orders">Bestellungen</th>
                ...
                <tbody class="divide-y divide-slate-200 font-medium">
                    @forelse($customers as $customer)
                        <tr class="exec-table-row hover:bg-slate-50/90 transition-colors duration-150 border-b border-slate-200/80">
                            ...
                            <td class="py-4 px-6 font-extrabold text-emerald-700 text-sm">
