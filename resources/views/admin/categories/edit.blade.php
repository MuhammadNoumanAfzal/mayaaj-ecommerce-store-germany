...
<div class="space-y-6 max-w-5xl mx-auto pb-16">
    ...
    <!-- Form Container Card -->
    <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" enctype="multipart/form-data" class="exec-card bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden border-t-4 border-t-indigo-600">
        @csrf
        @method('PUT')
        ...
        <div class="p-6 sm:p-8 text-xs space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                    <label class="block font-bold text-slate-700 mb-1" data-i18n-de="Neues Bild Hochladen (Optional)" data-i18n-en="Upload New Image (Optional)">Neues Bild Hochladen (Optional)</label>
                    <input 
                        type="file" 
                        name="image" 
                        accept="image/*" 
                    ...
                    @if($category->image_url)
                        <div class="mt-2.5 flex items-center gap-3 bg-slate-50 p-2 rounded-xl border border-slate-200 shadow-2xs">
                            <img src="{{ $category->image_url }}" alt="Aktuelles Bild" class="h-9 w-9 object-cover rounded-lg border border-slate-200 shadow-2xs">
